    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        if batch_idx % log_interval == 0:
            print('Train Epoch: {} [{}/{}]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(data_loader.dataset), loss.item()))
...
def main():
    ...
    transform = transforms.Compose([
        transforms.Resize((64, 64)),  # Resize images to match model input
        transforms.Grayscale(num_output_channels=1),  # Convert to grayscale (1 channel)
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))
    ])

    train_set = CIFAR10Dataset(train=True, transform=transform)
    test_set = CIFAR10Dataset(train=False, transform=transform)

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=False)
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=args.test_batch_size, shuffle=False, num_workers=args.num_workers)
